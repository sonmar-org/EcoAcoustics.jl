...
function parse_filename(path::AbstractString;
    ...
    if profile.filename_style == :tokenized && profile.split_char !== nothing
        ...
        get_token(idx::Union{Int,Nothing}) =
            isnothing(idx) || idx < 1 || idx > length(tokens) ? nothing : String(tokens[idx])
...
function _parse_datetime_from_tokens(tokens::Vector{<:AbstractString},
    ...
    # Strip the Z before parsing: Julia's DateFormat treats 'Z' as a timezone
    # offset code, so leaving it in dtstr causes parsing to fail for literal-Z
    # filenames like Rockhopper's "000654Z".
